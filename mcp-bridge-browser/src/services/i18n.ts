...
class I18nService {
    ...
    t(key: string): string {
        const translations: Record<string, { en: string; zh: string }> = {
            ...
            instr_search_marketplace: { en: 'Search in VS Code Marketplace:', zh: '在 VS Code 插件市场搜索：' },
            ...
            opt_no_tools: { en: 'No tools detected yet. Please use the extension once.', zh: '尚未检测到任何工具。请先在 AI 页面使用一次本插件。' },
            opt_prompts_title: { en: 'System Prompts', zh: '系统提示词 (Prompts)' },
            opt_init_label: { en: 'Initial System Prompt', zh: '初始系统提示词 (Initial)' },
            opt_init_desc: { en: 'Sent to AI when you start a new conversation. (Supports Markdown)', zh: '开启新对话时发送给 AI。支持 Markdown。' },
            opt_rules_label: { en: 'User Rules (Custom Preferences)', zh: '个人规则 (自定义偏好)' },
            opt_rules_desc: { en: 'Your personal requirements (e.g., "Always ask before coding"). Appended to System & Training prompts.', zh: '你的个性化要求（如“写代码前先询问”）。会追加到系统和训练提示词中。' },
            opt_train_label: { en: 'Training Hint (Periodic)', zh: '训练补丁 (周期性)' },
            opt_train_desc: { en: 'Inserted periodically (every 5 tool calls) to remind AI of the protocol.', zh: '每隔 5 次工具调用插入一次，提醒 AI 遵守协议。' },
            opt_error_label: { en: 'Format Error Hint', zh: '格式报错提示 (Error)' },
            opt_error_desc: { en: 'Sent to AI when it generates invalid JSON or fails to follow protocol.', zh: '当 AI 返回无效 JSON 或未遵守协议时发送。' },
            btn_reset: { en: 'Reset to Defaults', zh: '恢复默认设置' },
            btn_save: { en: 'Save Settings', zh: '保存设置' },
            status_saved_synced: { en: 'Settings saved & synced to VS Code!', zh: '设置已保存并同步至 VS Code！' },
            status_saved_local: { en: 'Saved locally (VS Code disconnected).', zh: '已保存至本地（VS Code 未连接）。' },
            ...
            confirm_reset: { en: 'Are you sure you want to reset ALL settings to defaults?', zh: '确定要将所有设置恢复为默认值吗？' },
        };
