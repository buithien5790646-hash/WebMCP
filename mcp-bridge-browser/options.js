...
const UI = {
  en: {
    ...
    desc_selectors: "Customize DOM selectors. Only modify if the extension stops working.",
    sec_prompts: "System Prompts",
    lbl_init_prompt: "Initial System Prompt",
    desc_init_prompt: "Sent to AI when you start a new conversation. (Supports Markdown)",
    ...
    reset_confirm: "Are you sure you want to reset ALL settings (Selectors & Prompts) to defaults?",
...
async function restoreOptions() {
  ...
  chrome.storage.local.get([KEY_PROMPT, KEY_TRAIN, KEY_ERROR], async (items) => {
    ...
    else els.initPrompt.value = await fetchDefault(lang === 'zh' ? 'prompt_zh.md' : 'prompt.md');

    // Train
    if (items[KEY_TRAIN]) els.trainPrompt.value = items[KEY_TRAIN];
    else els.trainPrompt.value = await fetchDefault(lang === 'zh' ? 'train_zh.md' : 'train.md');
    
    // Error
    if (items[KEY_ERROR]) els.errorPrompt.value = items[KEY_ERROR];
    else els.errorPrompt.value = await fetchDefault(lang === 'zh' ? 'error_hint_zh.md' : 'error_hint.md');
  });
}
