...
server.registerTool(
  "execute_command",
  {
    ...
    inputSchema: {
      ...
      cwd: z
        ...
        .optional()
        .describe(
          "Optional: Current working directory. Must be within the workspace. Defaults to workspace root."
        ),
      timeout: z
        .number()
        .default(60000)
        .describe("Optional: Timeout in milliseconds (default: 60s)."),
    },
  },
  async ({ command, cwd, timeout }) => {
    ...
    if (!projectRoot) {
      return {
        content: [
          {
            type: "text",
            text: `❌ Security Error: Project root not configured. Command execution is disabled for safety.`,
          },
        ],
        isError: true,
      };
      ...
      if (!isSubPath(projectRoot, resolvedCwd)) {
        return {
          content: [
            {
              type: "text",
              text: `❌ Permission Denied: Access to '${cwd}' is forbidden. Path must be within workspace: ${projectRoot}`,
            },
          ],
          isError: true,
        };
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `❌ Execution System Error: ${error.message}` }],
        isError: true,
      };
    }
  }
