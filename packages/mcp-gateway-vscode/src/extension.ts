...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mcp-gateway.toggleService", (id: string) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mcp-gateway.copyContext", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("mcp-gateway.connect", async () => {
      ...
      const items: CustomActionItem[] = [
        ...quickLaunchItems,
        {
          label: "$(run) Custom Launch...",
          description: "Select AI and Browser manually",
          action: "custom",
        },
        {
          label: "$(output) View Logs",
          description: "Show MCP Gateway output panel",
          action: "showLogs",
        },
        {
          label: "$(settings-gear) Configure Gateway",
          description: "Quick access to MCP Gateway settings",
          action: "settings",
        },
        {
          label: "$(refresh) Restart Server",
          description: "Restart local gateway",
          action: "restart",
        },
      ...
      if (selection.action === "custom") {
        ...
        const aiSelection = await vscode.window.showQuickPick<CustomActionItem>(
          aiOptionsForCustomLaunch,
          {
        ...
        const browserSelection = await vscode.window.showQuickPick<CustomActionItem>(
          browserOptions,
          {
