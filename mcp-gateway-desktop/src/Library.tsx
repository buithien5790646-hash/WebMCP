...
import { Trash2, Plus, Globe, Terminal, Save, X, CheckCircle2, AlertCircle, PlayCircle, Loader2, AlertTriangle, Download, ShoppingBag, LayoutList } from 'lucide-react';
...
export default function Library({ servers, envStatus, onReload }: Props) {
  ...
  const [installForm, setInstallForm] = useState<{ env: Record<string, string>, args: string[] }>({ env: {}, args: [] });
  ...
  const handleTest = async (server: ServerDefinition) => {
    ...
    setTestResults(prev => { const n = {...prev}; delete n[server.id]; return n; });
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="flex gap-2 mt-2">
              {Object.entries(envStatus).map(([tool, installed]) => (
                  <Badge key={tool} variant={installed ? "secondary" : "outline"} className={installed ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : "text-muted-foreground opacity-50"}>
                    {installed ? <CheckCircle2 className="w-3 h-3 mr-1" /> : <X className="w-3 h-3 mr-1" />}
                    {tool}
                  </Badge>
              ))}
            </div>
          ...
          <div className="flex bg-muted p-1 rounded-lg">
             <button 
                ...
                className={cn("px-4 py-2 text-sm font-medium rounded-md transition-all flex items-center gap-2", view === 'installed' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}
             >
                <LayoutList className="w-4 h-4" /> Installed
             </button>
             <button 
                onClick={() => setView('market')}
                className={cn("px-4 py-2 text-sm font-medium rounded-md transition-all flex items-center gap-2", view === 'market' ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}
             >
      ...
      {view === 'installed' && (
        <div className="space-y-4">
             {!isAdding && (
                <Button onClick={() => setIsAdding(true)} variant="outline" className="w-full border-dashed py-8">
            ...
            {isAdding && (
                ...
                <CardFooter className="justify-end gap-2">
                    ...
                    <Button onClick={handleManualSave}><Save className="mr-2 h-4 w-4"/> Save Definition</Button>
                </CardFooter>
                </Card>
            )}
                ...
                <Card key={server.id} className="flex items-center justify-between p-4 hover:bg-accent/5 transition-colors">
                    ...
                    <div>
                        ...
                        {testResults[server.id] && (
                            <div className={`text-xs flex items-center mt-2 ${testResults[server.id].ok ? 'text-emerald-600' : 'text-destructive'}`}>
              ...
              <Card className="w-full max-w-lg shadow-xl animate-in fade-in zoom-in duration-200">
                  <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => setInstallingItem(null)}><X className="h-4 w-4"/></Button>
                      </div>
                      <CardDescription>Please provide the necessary configuration to run this server.</CardDescription>
                  </CardHeader>
                        ...
                        {installingItem.variables?.env?.map(v => (
                            <div key={v.key} className="space-y-2">
                                ...
                                <Input 
                                    ...
                                    onChange={e => setInstallForm(prev => ({ ...prev, env: { ...prev.env, [v.key]: e.target.value } }))}
                                />
                  ...
                  <CardFooter className="justify-end gap-2 bg-muted/20">
                      <Button variant="ghost" onClick={() => setInstallingItem(null)}>Cancel</Button>
